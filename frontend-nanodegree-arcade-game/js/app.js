/* 	As the player, your mission is to reach the water without
 *	being "caught" by a bug.
 *
...
Enemy.prototype.checkCollision = function () {
	...
	if ( ( this.x + .75 * this.width >= player.x ) &&
		...
		( this.y <= player.y + .5 * player.height ) &&
		( this.y + .5 * this.height >= player.y )
	) {
 ...
 *		following:
 *			1. Makes player dance
 *      2. Plays success sound
 *			3. Increases the game level
 *			4. Gives the player another life
 *			5. Calculates the score for the level and adds to the total Score
 *			6. Resets the Player to the grassy area so they can start again
 *
 *
 *----------------------------------------------------------------------------*/
...
Player.prototype.goalAchieved = function () {
  //animates sprite celebration dance
  dance();
...
Player.prototype.update = function () {
  ...
  // console.log(`this.width: ${this.width}, this.height: ${this.height}, this.y: ${this.y}, maxHeight: ${maxHeight}, maxWidth: ${maxWidth}`);

	if ( this.y <= .001 * this.height ) {
...
/*******************************************************************************
	Global functions
      ...
      dance             - animates sprite, does a victory dance
 ******************************************************************************/
...
const gameOverVoice = new Sound( "sounds/whah_whah.wav" );
